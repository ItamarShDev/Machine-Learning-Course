...
class Cnn:
    def __init__(self):
        self.img_size = 28  # pixel size
        self.num_imgs = 1500  # images per class
        self.num_classes = 10
    ...
    def run(self):
        ...
        data_filename = self.download('notMNIST_small.tar.gz')
        print '\nData folders:'
        data_folders = self.untar(data_filename)

        # In[5]:

        dataset, labels = self.build_dataset(data_folders)
        ...
        train_ds = np.ndarray((train_size * self.num_classes, self.img_size, self.img_size), dtype=np.float32)
        train_lb = np.ndarray(train_size * self.num_classes, dtype=np.int32)

        valid_ds = np.ndarray((valid_size * self.num_classes, self.img_size, self.img_size), dtype=np.float32)
        valid_lb = np.ndarray(valid_size * self.num_classes, dtype=np.int32)

        test_ds = np.ndarray((test_size * self.num_classes, self.img_size, self.img_size), dtype=np.float32)
        test_lb = np.ndarray(test_size * self.num_classes, dtype=np.int32)

        # In[7]:

        for i in range(10):
            start_set, end_set = i * self.num_imgs, (i + 1) * self.num_imgs
        ...
        train_ds, train_lb = self.randomize(train_ds, train_lb)
        test_ds, test_lb = self.randomize(test_ds, test_lb)
        valid_ds, valid_lb = self.randomize(valid_ds, valid_lb)

        num_channels = 1  # grayscale

        def reformat(dataset, labels):
            # as.type is not needed as the array is already float32 but just in case
            dataset = dataset.reshape((-1, self.img_size, self.img_size, num_channels)).astype(np.float32)
        ...
        with self.graph.as_default():
            ...
            # Input data
            tf_train_ds = tf.placeholder(tf.float32,
                                         shape=(self.batch_size, self.img_size, self.img_size, self.num_channels))
            tf_train_lb = tf.placeholder(tf.float32, shape=(self.batch_size, self.num_classes))
            ...
            layer1_weights = tf.Variable(tf.truncated_normal(
                [self.img_size * self.img_size * self.num_channels, self.num_classes], stddev=0.1))
            layer1_biases = tf.Variable(tf.constant(1.0, shape=[self.num_classes]))
            ...
            def model(data, training):
                ...
                input_hidden = tf.reshape(data, [-1, self.img_size * self.img_size * num_channels])
            ...
            num_steps = 201  # the number of steps
            with tf.Session(graph=self.graph) as session:
                tf.initialize_all_variables().run()
                print('Initialized')
                for step in range(num_steps):
                    # randomize offset
                    offset = (step * self.batch_size) % (train_lb.shape[0] - self.batch_size)
                    batch_ds = train_ds[offset:(offset + self.batch_size)]
                    batch_lb = train_lb[offset:(offset + self.batch_size)]

                    feed_dict = {tf_train_ds: batch_ds, tf_train_lb: batch_lb}
                    _, l, predictions = session.run([optimizer, loss, train_prediction], feed_dict=feed_dict)
                    if step % 500 == 0:
                        print('Minibatch loss at step %d: %f' % (step, l))
                        print('Minibatch accuracy: %.1f%%' % self.accuracy(predictions, batch_lb))
                        print('Validation accuracy: %.1f%%' % self.accuracy(
                            valid_prediction.eval(), valid_lb))
                print('Test accuracy: %.1f%%' % self.accuracy(test_prediction.eval(), test_lb))

    def download(self, filename):
        """
        Download the files requested.
        ...
        if not os.path.exists(destination_file):
            ...
            urlretrieve(self.url + filename, destination_file)
        else:
            print "File already exists: %s" % filename
        return destination_file

    def build_dataset(self, folders):
        dataset = np.ndarray((self.num_imgs * self.num_classes, self.img_size, self.img_size),
                             dtype=np.float32)
        labels = np.ndarray(self.num_imgs * self.num_classes, dtype=np.int32)
        counter = 0
        for img_class, folder in enumerate(folders):
            per_class_counter = 0
            for img_name in os.listdir(folder):
                if per_class_counter < self.num_imgs:
