...
class AlexNet():
    def __init__(self):
        self.train_x = zeros((1, 227,227,3)).astype(float32)
        self.train_y = zeros((1, 1000))
        self.xdim = self.train_x.shape[1:]
        self.ydim = self.train_y.shape[1]
...
#Read Image
        ...
        self.x_dummy = (random.random((1,)+ self.xdim)/255.).astype(float32)
        self.i = self.x_dummy.copy()
        self.i[0, :, :, :] = (imread("poodle.png")[:, :, :3]).astype(float32)
...
################################################################################
    ...
    #         .softmax(name='prob'))
        self.net_data = load("bvlc_alexnet.npy").item()
        self.create_network()
